---
title: "Full log - issue #6 (Sep 20)"
date: 2026-09-20
---

Every torch.compile commit that landed between 2026-09-13 and 2026-09-20, including the ones the [issue]({{< relref "/posts/2026-09-20-this-week-in-torch-compile" >}}) only counted.
...
- Test-only change tagging free-tensor installation Dynamo tests with a hardware classification label for CI routing. ([#192870](https://github.com/pytorch/pytorch/pull/192870), @waterxyj)
- Test-only change tagging dictionary-related Dynamo tests with a hardware classification label for CI routing. ([#192706](https://github.com/pytorch/pytorch/pull/192706), @taochong123456)
- Internal refactor moving several symbolic-integer/float method implementations in Dynamo onto a shared declarative dispatch mechanism, with no behavior change. ([#195189](https://github.com/pytorch/pytorch/pull/195189), @PrabinDevkota)
...
- Test-only change tagging graph-helper tests with a hardware classification label for CI routing. ([#193916](https://github.com/pytorch/pytorch/pull/193916), @pengyeqing)
- Test-only refactor generalizing hardcoded CUDA/XPU device checks in a test file to work across accelerator backends and adding hardware classification labels. ([#193155](https://github.com/pytorch/pytorch/pull/193155), @wyb2939)
- Test-only change tagging block pointer store dtype tests with a hardware classification label for CI routing. ([#193918](https://github.com/pytorch/pytorch/pull/193918), @pengyeqing)
- Test-only change adding hardware classification labels across AOTInductor tests and splitting out compile-time-only tests into their own device-agnostic class. ([#193276](https://github.com/pytorch/pytorch/pull/193276), @Netlessfish)
- Test-only change tagging block analysis tests with a hardware classification label for CI routing. ([#193917](https://github.com/pytorch/pytorch/pull/193917), @pengyeqing)
